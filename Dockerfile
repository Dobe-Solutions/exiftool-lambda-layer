...
# copy config files
ARG CONFIG
COPY copyConfigFiles.sh exiftool-${EXIFTOOL_VERSION}
RUN cd exiftool-${EXIFTOOL_VERSION} && ./copyConfigFiles.sh ${CONFIG} ${PREFIX_PATH}
...
COPY --from=builder ${PREFIX_PATH}/config ${PREFIX_PATH}/config

# test installation before packaging
COPY sample.jpg ${PREFIX_PATH}
RUN ${PREFIX_PATH}/bin/exiftool -fast ${PREFIX_PATH}/sample.jpg

RUN cd ${PREFIX_PATH} && zip -r /tmp/exiftool-lambda-layer.zip bin/ lib/ config/